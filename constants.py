...
def is_image_generation_model(model_name):
    ...
    return model_name == "gemini-2.0-flash-preview-image-generation"
...
Я помню наш разговор (контекст), могу работать с файлами, использовать поиск Google и генерировать изображения!
...
🎨 *Генерировать изображения:* Выберите модель "2.0 Flash+IMG🎨" и просто опишите, что хотите увидеть! Могу создавать изображения по тексту, редактировать загруженные фото и делать серию изменений в диалоге.
...
HELP_TEXT_TEMPLATE = (
    ...
    "*Основной чат:*\n"
    "- Просто пишите мне, я помню контекст нашего диалога.\n"
    "- *Новый чат:* Начинает новый диалог, очищая весь предыдущий контекст (сообщения, файлы).\n"
    "- *Модель:* Позволяет выбрать ИИ-модель Gemini. Смена модели начинает новый чат.\n"
    "- *Поиск:* Включает/выключает поиск Google для более точных и актуальных ответов со ссылками на источники.\n\n"
    "*Генерация изображений (модель 2.0 Flash+IMG🎨):*\n"
    "- *Текст → Изображение:* «Нарисуй кота в космосе» → получите изображение\n"
    "- *Фото → Новое изображение:* Загрузите фото → «Измени стиль на винтаж»\n"
    "- *Серия изменений:* «Добавь шляпу» → «Измени цвет» → «Добавь фон»\n\n"
    "*Работа с файлами:*\n"
    "- Отправляйте фото (я опишу или учту) или документы (PDF, TXT, код и др., до {MAX_FILE_SIZE_MB} МБ).\n"
    "- Файлы добавляются в контекст *текущего чата* до его очистки ('Новый чат').\n"
    "- *В Мгновенном режиме:* Файлы из контекста автоматически прикрепляются к вашему *следующему текстовому* запросу.\n"
    "- *В Ручном режиме:* Файлы добавляются в буфер вместе с текстом/фото и отправляются все вместе по кнопке 'Отправить всё'.\n\n"
    "*Режимы отправки (кнопка 'Режим:'):*\n"
    "- *{SEND_MODE_IMMEDIATE}:* Ваше сообщение/фото/файл отправляется *сразу*. Удобно для быстрых вопросов и ответов.\n"
    "- *{SEND_MODE_MANUAL}:* Сообщения/фото/файлы *накапливаются* в буфере. Используйте эту опцию, чтобы:\n"
    "    - Отправить очень длинный текст, который не помещается в одно сообщение Telegram.\n"
    "    - Скомбинировать несколько фото, файлов и текстовых описаний в *один* запрос к Gemini.\n"
    "- *Отправить всё:* (Только в Ручном режиме) Отправляет всё содержимое буфера в Gemini.\n\n"
    "*Скачивание ответов:*\n"
    "- *Скачать .md:* (Кнопка на клавиатуре) Скачать *последний* ответ бота в формате Markdown (с форматированием заголовков, списков и т.д.).\n"
    "- *Скачать в формате .txt:* (Кнопка под сообщением, если ответ был разбит) Скачать полный текст ответа без Markdown-разметки.\n\n"
    "*Быстрые инструменты (Команды):*\n"
    "- Эти команды выполняют *одноразовые* задачи и не влияют на контекст вашего основного чата.\n"
)
