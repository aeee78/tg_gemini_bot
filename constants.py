...
MODEL_ALIASES = {
    ...
    "gemini-2.5-flash-image-preview": "2.5 Image 🎨",
}
...
HELP_TEXT_TEMPLATE = (
    ...
    "• *Поиск Google:* Включается в настройках для доступа к интернету.\n\n"
    "🎨 *Генерация изображений:*\n"
    "Переключитесь на модель *2.5 Image 🎨*.\n"
    ...
    "• Кнопка *«Настройки ⚙️»* позволяет включить/выключить *Поиск Google*.\n"
    f"• Кнопка *«Режим»* переключает способ отправки:\n"
    f"  1. *{SEND_MODE_IMMEDIATE}*: Я отвечаю сразу на каждое сообщение. Идеально для диалога.\n"
    f"  2. *{SEND_MODE_MANUAL}*: Сообщения копятся в буфер. Нажмите *«Отправить всё»*, чтобы послать их разом. Идеально для длинных промптов или анализа пачки файлов.\n\n"
    "💾 *Полезные мелочи:*\n"
    "• *Получить .MD:* Кнопка меню. Скачивает последний ответ файлами (.md и .txt).\n"
